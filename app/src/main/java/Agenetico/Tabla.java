...
import com.example.proyecto_grupal.R;
...
public class Tabla {
    ...
    public void agregarCabecera(Dataset datos)
    {
        ...
        String[] arraycabecera = datos.getRotulo().toArray(new String[0]);
     ...
     */
    public void agregarFilaTabla(ArrayList<String> elementos, Dataset datos)
    {
        TableRow.LayoutParams layoutCelda;
        TableRow.LayoutParams layoutFila = new TableRow.LayoutParams(TableRow.LayoutParams.WRAP_CONTENT, TableRow.LayoutParams.WRAP_CONTENT);
        TableRow fila = new TableRow(actividad);
       ...
       /* for(int i = 0; i< elementos.size(); i++)
        ...
        for(int i = 0; i< datos.getDataset().size(); i++)
        {
            TextView texto = new TextView(actividad);
            texto.setText(datos.getDataset().get(i).get(i).toString());
            texto.setGravity(Gravity.CENTER_HORIZONTAL);
            texto.setTextAppearance(actividad, R.style.estilo_celda);
            texto.setBackgroundResource(R.drawable.tabla_celda);
            layoutCelda = new TableRow.LayoutParams(obtenerAnchoPixelesTexto(texto.getText().toString()), TableRow.LayoutParams.WRAP_CONTENT);
